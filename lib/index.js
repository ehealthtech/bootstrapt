...
module.exports = function(opts) {
    ...
    //
    function tryToMergeFixture(into, fromPath, test) {
        ...
        try {
            ...
            test.fail(util.format('Unable to load fixture [%s] -> %s', filename, err.message));
        }
    }
    ...
    Promise.reduce(tests, function(prev, path) {
        ...
        return test(path, function(t) {
            ...
            // Merge the global fixtures, if any.
            //
            globalFixtures.forEach(function(gf) {
                tryToMergeFixture(fixture, Path.resolve(testDir, fixtureDir, gf), t);
            });
